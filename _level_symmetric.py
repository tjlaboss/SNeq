# Level Symmetric
#
# Adapted from a quadrature module borrowed from @wbinventor,
# who got it from @robertsj, who got it from...

import numpy


class LevelSymmetricQuadrature(object):
	...
	def getQuadratureSet(self, N):
		...
		quad["xi"] = numpy.zeros(per_octant)
		quad["weight"] = numpy.zeros(per_octant)
		
		# Loop over all angles in an octant
		m = 0
		for i in range(N2):
			for j in range(N2 - i):
				quad["mu"][m] = self.att[N][i]
				quad["eta"][m] = self.att[N][j]
				quad["xi"][m] = numpy.sqrt(1.0 - quad["mu"][m]**2 - quad["eta"][m]**2)
				quad["weight"][m] = self.wtt[N][self.wtt_loc[N][m]]*numpy.pi
				m += 1
